...
&BEGIN_PROVIDER [double precision, potential_c_beta_ao_new,(ao_num,ao_num,N_states)]
 ...
 double precision :: dtmp_c_a(3,N_states),dtmp_c_b(3,N_states)
  ...
  do j = 1, nucl_num
   ...
   !   call dgemm('N','T',ao_num,ao_num,n_points_integration_angular,1.d0,grad_ao_matrix_dvc_a(1,1,m,istate),size(grad_ao_matrix_dvc_a,1),ao_matrix(1,1),size(ao_matrix,1),1.d0,potential_c_alpha_ao_new(1,1,istate),size(potential_c_alpha_ao_new,1))
   !   call dgemm('N','T',ao_num,ao_num,n_points_integration_angular,1.d0,grad_ao_matrix_dvx_a(1,1,m,istate),size(grad_ao_matrix_dvx_a,1),ao_matrix(1,1),size(ao_matrix,1),1.d0,potential_x_alpha_ao_new(1,1,istate),size(potential_x_alpha_ao_new,1))
   !   call dgemm('N','T',ao_num,ao_num,n_points_integration_angular,1.d0,grad_ao_matrix_dvc_b(1,1,m,istate),size(grad_ao_matrix_dvc_b,1),ao_matrix(1,1),size(ao_matrix,1),1.d0,potential_c_beta_ao_new(1,1,istate),size(potential_c_beta_ao_new,1))
   !   call dgemm('N','T',ao_num,ao_num,n_points_integration_angular,1.d0,grad_ao_matrix_dvx_b(1,1,m,istate),size(grad_ao_matrix_dvx_b,1),ao_matrix(1,1),size(ao_matrix,1),1.d0,potential_x_beta_ao_new(1,1,istate),size(potential_x_beta_ao_new,1))
   !  enddo 
   ! enddo 
