 ...
 BEGIN_DOC
! Energy_c_md_mu_of_r_LDA = multi-determinantal correlation functional with mu(r) , see equation 40 in the original JCP of Giner (2018)
 END_DOC
 implicit none 
 integer :: i,istate 
...
&BEGIN_PROVIDER [double precision, Energy_c_md_on_top_PBE_mu_of_r, (N_states)]
 BEGIN_DOC
  ! Energy_c_md_on_top_PBE_mu_of_r_UEG_vector = PBE-on_top multi determinant functional with exact on top extracted from the UEG 
  ! Energy_c_md_on_top_PBE_mu_of_r_vector     = PBE-on_top multi determinant functional with exact on top extrapolated for large mu
 END_DOC
 implicit none
