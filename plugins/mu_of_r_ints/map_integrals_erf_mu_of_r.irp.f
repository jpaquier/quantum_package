...
BEGIN_PROVIDER [ double precision, ao_integrals_erf_mu_of_r_cache, (0:64*64*64*64) ]
  use map_module
 implicit none
 BEGIN_DOC
...
BEGIN_PROVIDER [ type(map_type), ao_integrals_erf_mu_of_r_map_left ]
  implicit none
  BEGIN_DOC
  ! AO integrals
  END_DOC
  ...
  call bielec_integrals_index(ao_num,ao_num,ao_num,ao_num,key_max)
  sze = key_max
  call map_init(ao_integrals_erf_mu_of_r_map_left,sze)
  print*,  'AO map initialized : ', sze
END_PROVIDER

 BEGIN_PROVIDER [ integer, ao_integrals_erf_mu_of_r_cache_min_left ]
&BEGIN_PROVIDER [ integer, ao_integrals_erf_mu_of_r_cache_max_lfet ]
 implicit none
 BEGIN_DOC
 ! Min and max values of the AOs for which the integrals are in the cache
 END_DOC
 ao_integrals_erf_mu_of_r_cache_min_left = max(1,ao_num - 63)
 ao_integrals_erf_mu_of_r_cache_max_lfet = ao_num

END_PROVIDER

BEGIN_PROVIDER [ double precision, ao_integrals_erf_mu_of_r_cache_left, (0:64*64*64*64) ]
  use map_module
 implicit none
 BEGIN_DOC
 ! Cache of AO integrals for fast access
 END_DOC
 PROVIDE ao_bielec_integrals_erf_mu_of_r_in_map_left
 integer                        :: i,j,k,l,ii
 integer(key_kind)              :: idx1,idx2
 real(integral_kind)            :: integral1,integral2
 !$OMP PARALLEL DO PRIVATE (i,j,k,l,idx1,idx2,ii,integral1,integral2)
 do l=ao_integrals_erf_mu_of_r_cache_min_left,ao_integrals_erf_mu_of_r_cache_max_lfet
   do k=ao_integrals_erf_mu_of_r_cache_min_left,ao_integrals_erf_mu_of_r_cache_max_lfet
     do j=ao_integrals_erf_mu_of_r_cache_min_left,ao_integrals_erf_mu_of_r_cache_max_lfet
       do i=ao_integrals_erf_mu_of_r_cache_min_left,ao_integrals_erf_mu_of_r_cache_max_lfet
         !DIR$ FORCEINLINE
         call bielec_integrals_index(i,j,k,l,idx1)
         !DIR$ FORCEINLINE
         call map_get(ao_integrals_erf_mu_of_r_map_left,idx1,integral1)
         ii = l-ao_integrals_erf_mu_of_r_cache_min_left
         ii = ior( ishft(ii,6), k-ao_integrals_erf_mu_of_r_cache_min_left)
         ii = ior( ishft(ii,6), j-ao_integrals_erf_mu_of_r_cache_min_left)
         ii = ior( ishft(ii,6), i-ao_integrals_erf_mu_of_r_cache_min_left)
         ao_integrals_erf_mu_of_r_cache_left(ii) = integral1 
       enddo
     enddo
   enddo
 enddo
...
double precision function get_ao_bielec_integral_erf_mu_of_r_left(i,j,k,l,map) result(result)
  use map_module
  implicit none
  BEGIN_DOC
  ! Gets one AO bi-electronic integral from the AO map
  ...
  integer                        :: ii
  real(integral_kind)            :: tmp
  PROVIDE ao_bielec_integrals_erf_mu_of_r_in_map_left ao_integrals_erf_mu_of_r_cache_left ao_integrals_erf_mu_of_r_cache_min_left
  !DIR$ FORCEINLINE
  if (ao_overlap_abs(i,k)*ao_overlap_abs(j,l) < ao_integrals_threshold ) then
    tmp = 0.d0
! else if (ao_bielec_integral_erf_mu_of_r_schwartz(i,k)*ao_bielec_integral_erf_mu_of_r_schwartz(j,l) < ao_integrals_threshold) then
!   tmp = 0.d0
    ...
    ii = l-ao_integrals_erf_mu_of_r_cache_min_left
    ii = ior(ii, k-ao_integrals_erf_mu_of_r_cache_min_left)
    ii = ior(ii, j-ao_integrals_erf_mu_of_r_cache_min_left)
    ii = ior(ii, i-ao_integrals_erf_mu_of_r_cache_min_left)
    if (iand(ii, -64) /= 0) then
      !DIR$ FORCEINLINE
      call bielec_integrals_index(i,j,k,l,idx)
      !DIR$ FORCEINLINE
      ...
      ii = ior( ishft(ii,6), j-ao_integrals_erf_mu_of_r_cache_min_left)
      ii = ior( ishft(ii,6), i-ao_integrals_erf_mu_of_r_cache_min_left)
      tmp = ao_integrals_erf_mu_of_r_cache_left(ii)
    endif
  endif
  result = tmp
end
...
subroutine get_ao_bielec_integrals_erf_mu_of_r_left(j,k,l,sze,out_val)
  use map_module
  BEGIN_DOC
  ! Gets multiple AO bi-electronic integral from the AO map .
  ! All i are retrieved for j,k,l fixed.
  ...
  integer(key_kind)              :: hash
  double precision               :: thresh
  PROVIDE ao_bielec_integrals_erf_mu_of_r_in_map_left ao_integrals_erf_mu_of_r_map_left
  thresh = ao_integrals_threshold
  ...
  double precision :: get_ao_bielec_integral_erf_mu_of_r_left
  do i=1,sze
    out_val(i) = get_ao_bielec_integral_erf_mu_of_r_left(i,j,k,l,ao_integrals_erf_mu_of_r_map_left) 
  enddo
  
end

subroutine get_ao_bielec_integrals_erf_mu_of_r_non_zero_left(j,k,l,sze,out_val,out_val_index,non_zero_int)
  use map_module
  implicit none
  BEGIN_DOC
  ! Gets multiple AO bi-electronic integral from the AO map .
  ...
  do i=1,sze
    integer, external :: ao_l4
    double precision, external :: ao_bielec_integral_erf_mu_of_r_left
    !DIR$ FORCEINLINE
    if (ao_overlap_abs(i,k)*ao_overlap_abs(j,l) < thresh) then
      cycle
    endif
    call bielec_integrals_index(i,j,k,l,hash1)
    call map_get(ao_integrals_erf_mu_of_r_map_left, hash1,tmp1)
    if (dabs(tmp1) < thresh ) cycle
    non_zero_int = non_zero_int+1
    out_val_index(non_zero_int) = i
    out_val(non_zero_int) = tmp1 
...
function get_ao_erf_mu_of_r_map_size_left()
  implicit none
  integer (map_size_kind) :: get_ao_erf_mu_of_r_map_size_left
  BEGIN_DOC
  ! Returns the number of elements in the AO map
  END_DOC
  get_ao_erf_mu_of_r_map_size_left = ao_integrals_erf_mu_of_r_map_left % n_elements
end

subroutine clear_ao_erf_mu_of_r_map_left
  implicit none
  BEGIN_DOC
  ! Frees the memory of the AO map
  END_DOC
  call map_deinit(ao_integrals_erf_mu_of_r_map_left)
  FREE ao_integrals_erf_mu_of_r_map_left
end
...
ao_integrals_erf_mu_of_r_map_left ; ao_integrals_erf_mu_of_r_left ; ao_num ;;
...
subroutine insert_into_ao_integrals_erf_mu_of_r_map_left(n_integrals,buffer_i, buffer_values)
  use map_module
  implicit none
  BEGIN_DOC
  ! Create new entry into AO map
  ...
  call map_append(ao_integrals_erf_mu_of_r_map_left, buffer_i, buffer_values, n_integrals)
end
