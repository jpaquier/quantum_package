subroutine ao_bielec_integrals_erf_mu_of_r_in_map_slave_tcp(i)
  implicit none
  integer, intent(in)            :: i
  BEGIN_DOC
! Computes a buffer of integrals. i is the ID of the current thread.
  END_DOC
  call ao_bielec_integrals_erf_mu_of_r_in_map_slave(0,i)
end


subroutine ao_bielec_integrals_erf_mu_of_r_in_map_slave_inproc(i)
  implicit none
  integer, intent(in)            :: i
  BEGIN_DOC
! Computes a buffer of integrals. i is the ID of the current thread.
  END_DOC
  call ao_bielec_integrals_erf_mu_of_r_in_map_slave(1,i)
end
...
subroutine ao_bielec_integrals_erf_mu_of_r_in_map_slave(thread,iproc)
  use map_module
  use f77_zmq
  implicit none
  BEGIN_DOC
...
subroutine ao_bielec_integrals_erf_mu_of_r_in_map_collector(zmq_socket_pull)
  use map_module
  use f77_zmq
  implicit none
  BEGIN_DOC
...
IRP_ENDIF
      ...
      call insert_into_ao_integrals_erf_mu_of_r_map(n_integrals,buffer_i,buffer_value)
      accu += n_integrals
      if (task_id /= 0) then
  ...
  control = get_ao_erf_mu_of_r_map_size(ao_integrals_erf_mu_of_r_map)
...
subroutine ao_bielec_integrals_erf_mu_of_r_in_map_slave_tcp_left(i)
  implicit none
  integer, intent(in)            :: i
  BEGIN_DOC
! Computes a buffer of integrals. i is the ID of the current thread.
  END_DOC
  call ao_bielec_integrals_erf_mu_of_r_in_map_slave_left(0,i)
end


subroutine ao_bielec_integrals_erf_mu_of_r_in_map_slave_inproc_left(i)
  implicit none
  integer, intent(in)            :: i
  BEGIN_DOC
! Computes a buffer of integrals. i is the ID of the current thread.
  END_DOC
  call ao_bielec_integrals_erf_mu_of_r_in_map_slave_left(1,i)
end
...
subroutine ao_bielec_integrals_erf_mu_of_r_in_map_slave_left(thread,iproc)
  use map_module
  use f77_zmq
  implicit none
  BEGIN_DOC
! Computes a buffer of integrals
  ...
  do 
    ...
    read(task,*) j, l
    integer, external :: task_done_to_taskserver
    call compute_ao_integrals_erf_mu_of_r_jl_left(j,l,n_integrals,buffer_i,buffer_value) 
    if (task_done_to_taskserver(zmq_to_qp_run_socket,worker_id,task_id) == -1) then
        stop 'Unable to send task_done'
    endif
    call push_integrals(zmq_socket_push, n_integrals, buffer_i, buffer_value, task_id)
...
subroutine ao_bielec_integrals_erf_mu_of_r_in_map_collector_left(zmq_socket_pull)
  use map_module
  use f77_zmq
  implicit none
  BEGIN_DOC
...
IRP_ENDIF
      ...
      call insert_into_ao_integrals_erf_mu_of_r_map_left(n_integrals,buffer_i,buffer_value)
      accu += n_integrals
      if (task_id /= 0) then
        integer, external :: zmq_delete_task
        if (zmq_delete_task(zmq_to_qp_run_socket,zmq_socket_pull,task_id,more) == -1) then
  ...
  integer (map_size_kind) :: get_ao_erf_mu_of_r_map_size 
  control = get_ao_erf_mu_of_r_map_size(ao_integrals_erf_mu_of_r_map_left)
