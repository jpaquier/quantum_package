 ...
 do i = 1, nucl_num
  ...
  do j = 1,Nucl_N_dirac_Aos(i) 
   ...
   do l = 1,dirac_ao_prim_num(k) 
    ...
    dirac_aos_array(k)+= dirac_ao_coef_normalized_ordered_transp_per_nucl(l,j,i) * dexp(-beta*r2) 
   enddo
   dirac_aos_array(k) = dirac_aos_array(k) * dx2 * dy2 * dz2
  enddo
 enddo
 end
