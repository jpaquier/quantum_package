...
! do i= 1, 2*large_ao_num+2*small_ao_num
!  if (i .le. large_ao_num) then
!   print*, i,j, dirac_mo_coef(i,j), dirac_ao_power(i,1), dirac_ao_power(i,2), dirac_ao_power(i,3) 
!  elseif (i .gt. large_ao_num .and. i .le. 2*large_ao_num) then
!   print*, i,j, dirac_mo_coef(i,j), dirac_ao_power(i-large_ao_num,1), dirac_ao_power(i-large_ao_num,2), dirac_ao_power(i-large_ao_num,3)
!  elseif (i .gt. 2*large_ao_num .and. i .le. 2*large_ao_num + small_ao_num) then
!   print*, i,j, dirac_mo_coef(i,j), dirac_ao_power(i-2*large_ao_num,1),dirac_ao_power(i-large_ao_num,2), dirac_ao_power(i-large_ao_num,3)
!  elseif (i .gt. large_ao_num + small_ao_num) then
!   print*, i,j, dirac_mo_coef(i,j), dirac_ao_power(i-large_ao_num-small_ao_num,1), dirac_ao_power(i-large_ao_num-small_ao_num,2), dirac_ao_power(i-large_ao_num-small_ao_num,3)
!  endif
! enddo
! print*,'************'
!enddo
...
!!!(Does not work for the orbitals corresponding to P-1 or P+1, and most likely not for D-1 and D+1) 
 do j= 2*small_ao_num+1, 2*small_ao_num + 2*large_ao_num-1 
