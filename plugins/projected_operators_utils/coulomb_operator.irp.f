subroutine expectation_value_in_real_space(r1,r2,coulomb,two_body_dm)
 BEGIN_DOC
 ! f_{\Psi}(r1,r2) = Equation A16 of J. Chem. Phys. 149, 194301 (2018)
...
double precision function integral_over_2_expectation_value_in_real_space(r)
 implicit none
! integral over 2 of pure_expectation_value_in_real_space(1,2) 
 ...
 integral_over_2_expectation_value_in_real_space = 0.d0
 do i = 1, mo_tot_num ! 1
  do j = 1, mo_tot_num ! 2 
   do m = 1, mo_tot_num ! 1
    do n = j,j           ! 2 :: kronecker(n,j) because of orthogonality of MOs
     !                                                                                 2 1 2 1
     integral_over_2_expectation_value_in_real_space += two_bod_alpha_beta_mo_contracted(n,m,j,i,1) * mos_array_r(m) * mos_array_r(i) 
    enddo
   enddo
  enddo
 enddo
...
subroutine expectation_value_in_real_space_for_hf(r1,r2,coulomb,two_bod)
 implicit none
 BEGIN_DOC
 ! f_{\Psi}(r1,r2) = Equation A16 of J. Chem. Phys. 149, 194301 (2018) with \Psi = HF
 END_DOC
 double precision, intent(in) :: r1(3), r2(3)
 double precision, intent(out):: coulomb,two_bod
 integer :: i,j,m,n
 integer :: ii,jj,mm,nn
 ...
 do m = 1, elec_alpha_num
  do n = 1, elec_beta_num
   ...
   do i = 1, mo_tot_num
    do j = 1, mo_tot_num
     coulomb +=  integrals_for_hf_potential(j,i,n,m) * mos_array_r1(i) * mos_array_r2(j) * mos_array_r2(n) * mos_array_r1(m) 
    enddo
   enddo
  enddo
 enddo
...
! integral_(r2,beta) f_HF(r1,r2) with f_HF being Equation A16 of J. Chem. Phys. 149, 194301 (2018) with \Psi = HF
 ...
 implicit none
 BEGIN_DOC 
! int dr1 dr2 f_HF(r1,r2) with f_HF being Equation A16 of J. Chem. Phys. 149, 194301 (2018) with \Psi = HF
