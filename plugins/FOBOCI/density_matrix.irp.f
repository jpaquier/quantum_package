   ...
   enddo
     ...
     do k=1,N_det_generators_restart
       ...
       do l=1,k-1
         ...
         do m=1,N_states
           ckl = psi_coef_generators_restart(k,m) * psi_coef_generators_restart(l,m) * phase
           if (s1==1) then
             tmp_a(h1,p1,m) += ckl
             tmp_a(p1,h1,m) += ckl
           else
     ...
     do k = 1, N_states
      print*, 'State ',k
      do i = 1, mo_tot_num
       print*,'DM restat',i,one_body_dm_mo_beta_generators_restart(i,i,k) + one_body_dm_mo_alpha_generators_restart(i,i,k)
      enddo
     enddo
