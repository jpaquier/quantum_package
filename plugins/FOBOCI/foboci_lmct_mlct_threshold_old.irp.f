...
subroutine FOBOCI_lmct_mlct_variational(iter)
  use bitmasks
 implicit none
 integer, intent(in) :: iter
 integer :: i,j,k,l
...
!  if(i_hole_osoci.ne.35)cycle
   ...
   call set_bitmask_hole_as_input(reunion_of_bitmask)
   double precision :: e_pt2
   call is_a_good_candidate_variational(threshold_lmct,is_ok,e_pt2,verbose,exit_loop,is_ok_perturbative)
   print*,'is_ok = ',is_ok
   if(is_ok)then
     call set_intermediate_normalization_lmct_old(norm_tmp,i_hole_osoci)
     call update_density_matrix_osoci
 ...
 !  ! Update the generators 
    ...
    call set_bitmask_hole_as_input(reunion_of_bitmask)
 !  ! so all the mono excitation on the new generators 
    call is_a_good_candidate_variational(threshold_mlct,is_ok,e_pt2,verbose,exit_loop,is_ok_perturbative)
    if(is_ok)then
      call set_intermediate_normalization_mlct_old(norm_tmp,i_particl_osoci)
      call update_density_matrix_osoci
    else
...
subroutine FOBOCI_lmct_only(iter)
  use bitmasks
 implicit none
 integer, intent(in) :: iter
 integer :: i,j,k,l
