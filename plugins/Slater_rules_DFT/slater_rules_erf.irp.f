...
subroutine i_H_j_mono_spin_mono(key_i,key_j,Nint,spin,hij)
  use bitmasks
  implicit none
  BEGIN_DOC
  ! Returns <i|H|j> where i and j are determinants differing by a single excitation
  ...
  call i_H_j_mono(key_i,key_j,Nint,hij)
! call get_mono_excitation_spin(key_i(1,spin),key_j(1,spin),exc,phase,Nint)
! call get_mono_excitation_from_fock(key_i,key_j,exc(1,1),exc(1,2),spin,phase,hij)
end

subroutine i_H_j_mono_spin_hartree(key_i,key_j,Nint,spin,hij)
  use bitmasks
  implicit none
  BEGIN_DOC
  ! Returns <i|H|j> where i and j are determinants differing by a single excitation
  ...
  call i_H_j_hartree(key_i,key_j,Nint,hij)
! call get_mono_excitation_spin(key_i(1,spin),key_j(1,spin),exc,phase,Nint)
! call get_mono_excitation_from_fock(key_i,key_j,exc(1,1),exc(1,2),spin,phase,hij)
end
