BEGIN_PROVIDER [double precision, super_ci_density_matrix_mo, (mo_tot_num,mo_tot_num,N_states)]
 implicit none
 ...
 do m = 1, N_states
   do i = 1,n_core_inact_orb
    ...
    super_ci_density_matrix_mo(iorb,iorb,m) = 2.d0
    do j = 1, n_virt_orb
     jorb = list_virt(j)
     index_ci = index_rotation_CI(i,j)
    ! DIAGONAL PART OF THE OCCUPIED 
     super_ci_density_matrix_mo(iorb,iorb,m) -= eigenvectors_sci(index_ci,m) **2 
    ! OCC-VIRT PART
     super_ci_density_matrix_mo(iorb,jorb,m) += eigenvectors_sci(1,m) * eigenvectors_sci(index_ci,m) * dsqrt_2
     super_ci_density_matrix_mo(jorb,iorb,m) += eigenvectors_sci(1,m) * eigenvectors_sci(index_ci,m) * dsqrt_2
    enddo
   enddo
   ...
   do i = 1,n_core_inact_orb
    ...
    do j = i+1, n_core_inact_orb
     ...
     do k = 1, n_virt_orb
      ...
      super_ci_density_matrix_mo(iorb,jorb,m) -= eigenvectors_sci(index_ci,m) * eigenvectors_sci(index_cj,m)
      super_ci_density_matrix_mo(jorb,iorb,m) -= eigenvectors_sci(index_ci,m) * eigenvectors_sci(index_cj,m)
     enddo
    enddo
   enddo
   ...
   do i = 1, n_act_orb
    ...
    do j = 1, n_act_orb
     jorb = list_act(j)
     super_ci_density_matrix_mo(iorb,jorb,m) = one_body_dm_mo_alpha(iorb,jorb,m) + one_body_dm_mo_beta(iorb,jorb,m)
    enddo
   enddo
...
BEGIN_PROVIDER [double precision, super_ci_state_average_density_matrix_mo, (mo_tot_num,mo_tot_num)]
  ...
  do i = 1, N_states
   super_ci_state_average_density_matrix_mo += super_ci_density_matrix_mo(:,:,i) * state_average_weight(i)
  enddo
  n_elec_states = 0.d0
  n_elec_average = 0.d0
  do j = 1, mo_tot_num
   n_elec_average += super_ci_state_average_density_matrix_mo(j,j)
   do i = 1, N_states
    n_elec_states(i) += super_ci_density_matrix_mo(j,j,i)
   enddo
  enddo
