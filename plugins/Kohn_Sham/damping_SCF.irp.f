subroutine damping_SCF
  ...
  do j=1,ao_num
    do i=1,ao_num
      D_alpha(i,j) = KS_density_matrix_ao_alpha(i,j)
      D_beta (i,j) = KS_density_matrix_ao_beta (i,j)
    enddo
  enddo
  ...
  do k=1,n_it_scf_max
    ...
    D_alpha = KS_density_matrix_ao_alpha
    D_beta  = KS_density_matrix_ao_beta 
    mo_coef = eigenvectors_fock_matrix_mo
    TOUCH mo_coef
    
    D_new_alpha = KS_density_matrix_ao_alpha
    D_new_beta  = KS_density_matrix_ao_beta 
    ...
    E_half = 0.d0
    do while (E_half > E)
      KS_density_matrix_ao_alpha = D_alpha + lambda * delta_alpha
      KS_density_matrix_ao_beta  = D_beta  + lambda * delta_beta
      TOUCH KS_density_matrix_ao_alpha KS_density_matrix_ao_beta
      mo_coef = eigenvectors_fock_matrix_mo
      TOUCH mo_coef
      E_half = KS_energy
    ...
    do j=1,ao_num
      do i=1,ao_num
        delta_D = delta_D + &
        (D_alpha(i,j) - KS_density_matrix_ao_alpha(i,j))*(D_alpha(i,j) - KS_density_matrix_ao_alpha(i,j)) + &
        (D_beta (i,j) - KS_density_matrix_ao_beta (i,j))*(D_beta (i,j) - KS_density_matrix_ao_beta (i,j))
      enddo
    enddo
    delta_D = dsqrt(delta_D/dble(ao_num)**2)
    KS_density_matrix_ao_alpha = D_alpha
    KS_density_matrix_ao_beta  = D_beta
    TOUCH KS_density_matrix_ao_alpha KS_density_matrix_ao_beta
    mo_coef = eigenvectors_fock_matrix_mo
    TOUCH mo_coef
