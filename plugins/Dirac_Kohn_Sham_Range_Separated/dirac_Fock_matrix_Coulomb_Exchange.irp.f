 BEGIN_PROVIDER [ complex*16, dirac_ao_bi_elec_Coulomb_Exchange_integral, (2*dirac_ao_num, 2*dirac_ao_num) ]
  use map_module
  implicit none
  BEGIN_DOC
  ...
  complex*16, allocatable        :: dirac_ao_bi_elec_Coulomb_Exchange_integral_tmp(:,:)
  ...
  dirac_ao_bi_elec_Coulomb_Exchange_integral = (0.d0,0.d0)
!!$OMP PARALLEL DEFAULT(NONE)                                      &
!!$OMP PRIVATE(i,j,l,k1,k,integral,ii,jj,kk,ll,i8,keys,values,n_elements_max, &
!!$OMP  n_elements,dirac_ao_bi_elec_Coulomb_Exchange_integral_tmp)&
!!$OMP SHARED(dirac_ao_num,dirac_SCF_density_matrix_ao,&
!!$OMP  dirac_ao_integrals_map, dirac_ao_bi_elec_Coulomb_Exchange_integral) 
  call get_cache_map_n_elements_max(dirac_ao_integrals_erf_map,n_elements_max)
  allocate(keys(n_elements_max), values(n_elements_max))
  allocate(dirac_ao_bi_elec_Coulomb_Exchange_integral_tmp(2*dirac_ao_num,2*dirac_ao_num))
  dirac_ao_bi_elec_Coulomb_Exchange_integral_tmp = (0.d0,0.d0)
   ...
   do k1=1,n_elements
    ...
    do k2=1,8
     ...
     if ((i .le. large_ao_num .and. j .le. large_ao_num .and. k .le. large_ao_num .and. l .le. large_ao_num) .or.  &
      ...
      dirac_ao_bi_elec_Coulomb_Exchange_integral_tmp(dirac_inverse_list(i,1),dirac_inverse_list(l,1)) -= (dirac_SCF_density_matrix_ao(dirac_inverse_list(j,1),dirac_inverse_list(k,1))) * integral
      !L_beta L_beta .or. S_beta S_beta
      dirac_ao_bi_elec_Coulomb_Exchange_integral_tmp(dirac_inverse_list(i,2),dirac_inverse_list(l,2)) -= (dirac_SCF_density_matrix_ao(dirac_inverse_list(j,2),dirac_inverse_list(k,2))) * integral   
      !L_beta L_alpha .or. S_beta S_alpha
      dirac_ao_bi_elec_Coulomb_Exchange_integral_tmp(dirac_inverse_list(i,2),dirac_inverse_list(l,1)) -= (dirac_SCF_density_matrix_ao(dirac_inverse_list(j,2),dirac_inverse_list(k,1))) * integral
      !L_alpha L_beta .or. S_alpha S_beta
      dirac_ao_bi_elec_Coulomb_Exchange_integral_tmp(dirac_inverse_list(i,1),dirac_inverse_list(l,2)) -= (dirac_SCF_density_matrix_ao(dirac_inverse_list(j,1),dirac_inverse_list(k,2))) * integral
...
!!$OMP END DO NOWAIT
!!$OMP CRITICAL
  dirac_ao_bi_elec_Coulomb_Exchange_integral += dirac_ao_bi_elec_Coulomb_Exchange_integral_tmp
!!$OMP END CRITICAL
  deallocate(keys,values,dirac_ao_bi_elec_Coulomb_Exchange_integral_tmp)
!!$OMP END PARALLEL
 END_PROVIDER
