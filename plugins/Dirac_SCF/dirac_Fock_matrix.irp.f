   ...
   do k1=1,n_elements
    ...
    do k2=1,8
     ...
     if ((i .le. large_ao_num .and. j .le. large_ao_num .and. k .le. large_ao_num .and. l .le. large_ao_num) .or.  &
      ...
      dirac_ao_bi_elec_integral_tmp(d_I(i,1),d_I(j,1)) += (dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,1))) * integral 
      dirac_ao_bi_elec_integral_tmp(d_I(i,1),d_I(j,1)) += (dirac_SCF_density_matrix_ao(d_I(l,2),d_I(k,2))) * integral
      dirac_ao_bi_elec_integral_tmp(d_I(i,1),d_I(l,1)) -= (dirac_SCF_density_matrix_ao(d_I(j,1),d_I(k,1))) * integral
      !L_beta L_beta .or. S_beta S_beta
      dirac_ao_bi_elec_integral_tmp(d_I(i,2),d_I(j,2)) += (dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,1))) * integral
      dirac_ao_bi_elec_integral_tmp(d_I(i,2),d_I(j,2)) += (dirac_SCF_density_matrix_ao(d_I(l,2),d_I(k,2))) * integral
      dirac_ao_bi_elec_integral_tmp(d_I(i,2),d_I(l,2)) -= (dirac_SCF_density_matrix_ao(d_I(j,2),d_I(k,2))) * integral   
      !L_beta L_alpha .or. S_beta S_alpha
      dirac_ao_bi_elec_integral_tmp(d_I(i,2),d_I(l,1)) -= (dirac_SCF_density_matrix_ao(d_I(j,2),d_I(k,1))) * integral
      !L_alpha L_beta .or. S_alpha S_beta
      dirac_ao_bi_elec_integral_tmp(d_I(i,1),d_I(l,2)) -= (dirac_SCF_density_matrix_ao(d_I(j,1),d_I(k,2))) * integral
...
!!$OMP END PARALLEL
 ...
 BEGIN_PROVIDER [ complex*16, dirac_ao_bi_elec_integral_erf, (2*dirac_ao_num, 2*dirac_ao_num) ]
  use map_module
  implicit none
  BEGIN_DOC
  ...
  double precision               :: integral
  complex*16, allocatable        :: dirac_ao_bi_elec_integral_erf_tmp(:,:)
  integer(omp_lock_kind)         :: lck(dirac_ao_num)
  integer*8                      :: i8
  integer                        :: ii(8), jj(8), kk(8), ll(8), k2
  integer(cache_map_size_kind)   :: n_elements_max, n_elements
  integer(key_kind), allocatable :: keys(:)
  double precision, allocatable  :: values(:)
  dirac_ao_bi_elec_integral_erf = (0.d0,0.d0)
!!$OMP PARALLEL DEFAULT(NONE)                                      &
!!$OMP PRIVATE(i,j,l,k1,k,integral,ii,jj,kk,ll,i8,keys,values,n_elements_max, &
!!$OMP  n_elements,dirac_ao_bi_elec_integral_erf_tmp)&
!!$OMP SHARED(dirac_ao_num,dirac_SCF_density_matrix_ao,&
!!$OMP  dirac_ao_integrals_erf_map, dirac_ao_bi_elec_integral_erf) 
  call get_cache_map_n_elements_max(dirac_ao_integrals_erf_map,n_elements_max)
  allocate(keys(n_elements_max), values(n_elements_max))
  allocate(dirac_ao_bi_elec_integral_erf_tmp(2*dirac_ao_num,2*dirac_ao_num))
  dirac_ao_bi_elec_integral_erf_tmp = (0.d0,0.d0)
!!$OMP DO SCHEDULE(dynamic,64)
!!DIR$ NOVECTOR
   ...
   do k1=1,n_elements
    ...
    do k2=1,8
     ...
     if ((i .le. large_ao_num .and. j .le. large_ao_num .and. k .le. large_ao_num .and. l .le. large_ao_num) .or.  &
         (i .gt. large_ao_num .and. j .gt. large_ao_num .and. k .gt. large_ao_num .and. l .gt. large_ao_num)) then
      !L_alpha L_alpha .or. S_alpha S_alpha
      dirac_ao_bi_elec_integral_erf_tmp(d_I(i,1),d_I(j,1)) += (dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,1))) * integral 
      dirac_ao_bi_elec_integral_erf_tmp(d_I(i,1),d_I(j,1)) += (dirac_SCF_density_matrix_ao(d_I(l,2),d_I(k,2))) * integral
      dirac_ao_bi_elec_integral_erf_tmp(d_I(i,1),d_I(l,1)) -= (dirac_SCF_density_matrix_ao(d_I(j,1),d_I(k,1))) * integral
      !L_beta L_beta .or. S_beta S_beta
      dirac_ao_bi_elec_integral_erf_tmp(d_I(i,2),d_I(j,2)) += (dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,1))) * integral
      dirac_ao_bi_elec_integral_erf_tmp(d_I(i,2),d_I(j,2)) += (dirac_SCF_density_matrix_ao(d_I(l,2),d_I(k,2))) * integral
      dirac_ao_bi_elec_integral_erf_tmp(d_I(i,2),d_I(l,2)) -= (dirac_SCF_density_matrix_ao(d_I(j,2),d_I(k,2))) * integral   
      !L_beta L_alpha .or. S_beta S_alpha
      dirac_ao_bi_elec_integral_erf_tmp(d_I(i,2),d_I(l,1)) -= (dirac_SCF_density_matrix_ao(d_I(j,2),d_I(k,1))) * integral
      !L_alpha L_beta .or. S_alpha S_beta
      dirac_ao_bi_elec_integral_erf_tmp(d_I(i,1),d_I(l,2)) -= (dirac_SCF_density_matrix_ao(d_I(j,1),d_I(k,2))) * integral
     elseif((i .le. large_ao_num .and. j .le. large_ao_num .and. k .gt. large_ao_num .and. l .gt. large_ao_num) .or. &
            (i .gt. large_ao_num .and. j .gt. large_ao_num .and. k .le. large_ao_num .and. l .le. large_ao_num))then
       !L_alpha L_alpha .or. S_alpha S_alpha
      dirac_ao_bi_elec_integral_erf_tmp(d_I(i,1),d_I(j,1)) += (dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,1))) * integral
...
!!$OMP END DO NOWAIT
!!$OMP CRITICAL
  dirac_ao_bi_elec_integral_erf += dirac_ao_bi_elec_integral_erf_tmp
!!$OMP END CRITICAL
  deallocate(keys,values,dirac_ao_bi_elec_integral_erf_tmp)
!!$OMP END PARALLEL
 END_PROVIDER
...
!!$OMP END PARALLEL
 ...
 BEGIN_PROVIDER [ complex*16, dirac_ao_bi_elec_Gaunt_integral_erf, (2*dirac_ao_num, 2*dirac_ao_num) ]
  use map_module
  implicit none
  BEGIN_DOC
  !Array of the bi-electronic Fock matrix for the Coulomb-Gaunt interaction,
  ...
  double precision               :: dirac_ao_bielec_integral_erf, local_threshold
  double precision               :: integral
  complex*16, allocatable        :: dirac_ao_bi_elec_Gaunt_integral_erf_tmp(:,:)
  integer(omp_lock_kind)         :: lck(dirac_ao_num)
  integer*8                      :: i8
  integer                        :: ii(8), jj(8), kk(8), ll(8), k2
  integer(cache_map_size_kind)   :: n_elements_max, n_elements
  integer(key_kind), allocatable :: keys(:)
  double precision, allocatable  :: values(:)
  dirac_ao_bi_elec_Gaunt_integral_erf = (0.d0,0.d0)
!!$OMP PARALLEL DEFAULT(NONE)                                      &
!!$OMP PRIVATE(i,j,l,k1,k,integral,ii,jj,kk,ll,i8,keys,values,n_elements_max, &
!!$OMP  n_elements,dirac_ao_bi_elec_Gaunt_integral_erf_tmp)&
!!$OMP SHARED(dirac_ao_num,dirac_SCF_density_matrix_ao,&
!!$OMP  dirac_ao_integrals_map, dirac_ao_bi_elec_Gaunt_integral_erf) 
  call get_cache_map_n_elements_max(dirac_ao_integrals_erf_map,n_elements_max)
  allocate(keys(n_elements_max), values(n_elements_max))
  allocate(dirac_ao_bi_elec_Gaunt_integral_erf_tmp(2*dirac_ao_num,2*dirac_ao_num))
  dirac_ao_bi_elec_Gaunt_integral_erf_tmp = (0.d0,0.d0)
!!$OMP DO SCHEDULE(dynamic,64)
!!DIR$ NOVECTOR
  do i8=0_8,dirac_ao_integrals_erf_map%map_size
   n_elements = n_elements_max
 ...
 !!! Coulomb part of the interaction !!!
     if ((i .le. large_ao_num .and. j .le. large_ao_num .and. k .le. large_ao_num .and. l .le. large_ao_num) .or.  &
         (i .gt. large_ao_num .and. j .gt. large_ao_num .and. k .gt. large_ao_num .and. l .gt. large_ao_num)) then
      !L_alpha L_alpha .or. S_alpha S_alpha
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,1),d_I(j,1)) += (dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,1))) * integral 
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,1),d_I(j,1)) += (dirac_SCF_density_matrix_ao(d_I(l,2),d_I(k,2))) * integral
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,1),d_I(l,1)) -= (dirac_SCF_density_matrix_ao(d_I(j,1),d_I(k,1))) * integral
      !L_beta L_beta .or. S_beta S_beta
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,2),d_I(j,2)) += (dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,1))) * integral
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,2),d_I(j,2)) += (dirac_SCF_density_matrix_ao(d_I(l,2),d_I(k,2))) * integral
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,2),d_I(l,2)) -= (dirac_SCF_density_matrix_ao(d_I(j,2),d_I(k,2))) * integral   
      !L_beta L_alpha .or. S_beta S_alpha
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,2),d_I(l,1)) -= (dirac_SCF_density_matrix_ao(d_I(j,2),d_I(k,1))) * integral
      !L_alpha L_beta .or. S_alpha S_beta
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,1),d_I(l,2)) -= (dirac_SCF_density_matrix_ao(d_I(j,1),d_I(k,2))) * integral
     elseif((i .le. large_ao_num .and. j .le. large_ao_num .and. k .gt. large_ao_num .and. l .gt. large_ao_num) .or. &
            (i .gt. large_ao_num .and. j .gt. large_ao_num .and. k .le. large_ao_num .and. l .le. large_ao_num))then
       !L_alpha L_alpha .or. S_alpha S_alpha
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,1),d_I(j,1)) += (dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,1))) * integral
 ...
 !!! Gaunt part of the interaction !!!
     if ((i .le. large_ao_num .and. j .gt. large_ao_num .and. k .gt. large_ao_num .and. l .le. large_ao_num)  .or.  &
         (i .gt. large_ao_num .and. j .le. large_ao_num .and. k .le. large_ao_num .and. l .gt. large_ao_num)) then  
      !L_alpha L_alpha .or. S_alpha S_alpha
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,1),d_I(l,1)) += 2*(dirac_SCF_density_matrix_ao(d_I(j,2),d_I(k,2))) * integral 
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,1),d_I(l,1)) += (dirac_SCF_density_matrix_ao(d_I(j,1),d_I(k,1))) * integral
      !L_beta L_beta .or. S_beta S_beta
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,2),d_I(l,2)) += 2*(dirac_SCF_density_matrix_ao(d_I(j,1),d_I(k,1))) * integral
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,2),d_I(l,2)) += (dirac_SCF_density_matrix_ao(d_I(j,2),d_I(k,2))) * integral
      !L_beta L_alpha .or. S_beta S_alpha
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,2),d_I(l,1)) -= (dirac_SCF_density_matrix_ao(d_I(j,2),d_I(k,1))) * integral
      !L_alpha L_beta .or. S_alpha S_beta
      ...
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,2),d_I(j,2)) += (dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,1))) * integral
      !L_beta S_alpha .or S_beta L_alpha
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,2),d_I(j,1)) -= 2*(dirac_SCF_density_matrix_ao(d_I(l,2),d_I(k,1))) * integral
      !L_alpha S_beta .or S_alpha L_beta
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,1),d_I(j,2)) -= 2*(dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,2))) * integral
     elseif ((i .le. large_ao_num .and. j .gt. large_ao_num .and. k .le. large_ao_num .and. l .gt. large_ao_num)  .or.  &
             (i .gt. large_ao_num .and. j .le. large_ao_num .and. k .gt. large_ao_num .and. l .le. large_ao_num)) then
      !L_alpha S_alpha .or S_alpha L_alpha
      dirac_ao_bi_elec_Gaunt_integral_erf_tmp(d_I(i,1),d_I(j,1)) -= (dirac_SCF_density_matrix_ao(d_I(l,1),d_I(k,1))) * integral
...
!!$OMP END DO NOWAIT
!!$OMP CRITICAL
  dirac_ao_bi_elec_Gaunt_integral_erf += dirac_ao_bi_elec_Gaunt_integral_erf_tmp
!!$OMP END CRITICAL
  deallocate(keys,values,dirac_ao_bi_elec_Gaunt_integral_erf_tmp)
!!$OMP END PARALLEL
 END_PROVIDER
