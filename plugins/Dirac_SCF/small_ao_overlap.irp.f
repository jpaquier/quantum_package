   ...
   do i= 1, small_ao_num
    ...
    do n = 1,small_ao_prim_num(j)
     ...
     do l = 1, small_ao_prim_num(i)
      ...
      call overlap_gaussian_xyz(A_center,B_center,alpha,beta,power_A,power_B,overlap_x,overlap_y,overlap_z,small_overlap,dim1)
   ...
   do i= 1, small_ao_num
    ...
    do n = 1,ao_prim_num(j)
     ...
     do l = 1, ao_prim_num(i)
      beta  = small_ao_expo_ordered_transp(l,i)
      call overlap_x_abs(A_center(1),B_center(1),α,β,power_A(1),power_B(1),overlap_x,lower_exp_val,dx,dim1)
      call overlap_x_abs(A_center(2),B_center(2),α,β,power_A(2),power_B(2),overlap_y,lower_exp_val,dx,dim1)
      call overlap_x_abs(A_center(3),B_center(3),α,β,power_A(3),power_B(3),overlap_z,lower_exp_val,dx,dim1)
      c = small_ao_coef_normalized_ordered_transp(n,j) * small_ao_coef_normalized_ordered_transp(l,i)
      ao_overlap_abs(i,j) += abs(c) * overlap_x * overlap_y * overlap_z
     enddo
  enddo
 END_PROVIDER
