 ...
 implicit none
  ...
  do i= 1,small_ao_num
   ...
   do j=1,ao_num
    B_center(1) = nucl_coord( ao_nucl(j), 1 )
    B_center(2) = nucl_coord( ao_nucl(j), 2 )
    B_center(3) = nucl_coord( ao_nucl(j), 3 )
    power_B(1)  = ao_power( j, 1 )
    power_B(2)  = ao_power( j, 2 )
    power_B(3)  = ao_power( j, 3 )
    do l = 1, small_ao_prim_num(i)
     alpha   = small_ao_expo_ordered_transp(l,i)
     do n = 1,ao_prim_num(j)
      beta  = ao_expo_ordered_transp(n,j)
      ...
      power_B(1) = power_B(1)-1   
      small_ao_deriv1_x(i,j) += c*(Power_B(1)*deriv_tmp1 - 2.d0*beta*deriv_tmp2) * overlap_y0 * overlap_z0
      ...
      power_B(2) = power_B(2)-1   
      small_ao_deriv1_y(i,j) += c*(Power_B(2)*deriv_tmp1 - 2.d0*beta*deriv_tmp2) * overlap_x0 * overlap_z0
      ...
      power_B(3) = power_B(3)-1   
      small_ao_deriv1_z(i,j) += c*(Power_B(3)*deriv_tmp1 - 2.d0*beta*deriv_tmp2) * overlap_x0 * overlap_y0
     enddo 
    enddo
   enddo
  enddo
