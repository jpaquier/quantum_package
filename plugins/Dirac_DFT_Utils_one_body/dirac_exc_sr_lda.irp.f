!subroutine dirac_ex_lda(rho,e_x,v_x)
...
! e_x =    0.001343813934716645*kF**4*(-2.d0 + tmp_c**2 - 2.d0*Sqrt(1.d0 + tmp_c**2)*(2.d0 + 3.d0*tmp_c**2)*ArcCsch(tmp_c)    &
!         + 3.d0*tmp_c**4*ArcCsch(tmp_c)**2 + 2.d0*(1.d0 + tmp_c**2)**2*Log(1.d0 + tmp_c**(-2)))
!
! v_x =   (0.1061032953945969*kF*(-1.d0*Sqrt(1.d0 + tmp_c**2)*(2.d0 + 3.d0*tmp_c**2)*ArcCsch(tmp_c) +                        &
!        (1.d0 + tmp_c**2)*(-1.d0 + (1.d0 + tmp_c**2)*Log(1.d0 + tmp_c**(-2)))))/(1.d0 + tmp_c**2)
!endif
!end

 subroutine dirac_ec_lda(rho,e_c,v_c)
      implicit none
 include 'constants.include.F'
 ...
 if (dirac_interaction == "Coulomb") then
  if (dirac_exchange_functional == "dirac_short_range_LDA_P2") then
   if (rho .lt. 1.e-8) then

    e_x = 0.07957747154594767d0*kF*(-3.d0 + (-6.d0 + 4.d0/dexp(1.d0/mu_erf**2))*mu_erf**2 + (2.d0 - 2.d0/dexp(1.d0/mu_erf**2))*mu_erf**4 + 7.089815403622064d0*mu_erf*Erf(1.d0/mu_erf))  

    v_x =  (0.7853981633974483d0*(-3.d0 + (-6.d0 + 4.d0/dexp(1.d0/mu_erf**2))*mu_erf**2 + (2.d0 - 2.d0/dexp(1.d0/mu_erf**2))*mu_erf**4 + 7.089815403622064d0*mu_erf*Erf(1.d0/mu_erf)))/kF**2 
 
   else

    e_x = (0.00004479379782388818d0*kF**4*(60.d0*(-3.d0 + 2.d0*tmp_mu**2*(-3.d0 + tmp_mu**2 - (1.d0*(-2.d0 + tmp_mu**2))*dexp(-1.d0/tmp_mu**2)) +                          &     
         7.089815403622064d0*tmp_mu*Erf(1.d0/tmp_mu)) + (20.d0*(1.d0 + 6.d0*tmp_mu**4*(3.d0 - 2.d0*tmp_mu**2 + (-1.d0 + 2.d0*tmp_mu**2)*dexp(-1.d0/tmp_mu**2)) -          &
         10.6347231054331d0*tmp_mu**3*Erf(1.d0/tmp_mu))**2 - (3.d0*(-2.d0*tmp_mu**2*(-2.d0 + tmp_mu**2) + dexp(1.d0/tmp_mu**2)*(-3.d0 - 6.d0*tmp_mu**2 +                  &
         2.d0*tmp_mu**4 + 7.089815403622064d0*tmp_mu*Erf(1.d0/tmp_mu)))*(24.d0*tmp_mu**4*(4.d0 + 13.d0*tmp_mu**2 - 27.d0*tmp_mu**4) + dexp(1.d0/tmp_mu**2)*(-13.d0 -      &
         540.d0*tmp_mu**4 - 960.d0*tmp_mu**6 + 648.d0*tmp_mu**8 + 21.26944621086619d0*tmp_mu**3*(8.d0 + 45.d0*tmp_mu**2)*Erf(1.d0/tmp_mu))))*dexp(-2.d0/tmp_mu**2))/      &
         ...
         648.d0*tmp_mu**8 + 21.26944621086619d0*tmp_mu**3*(8.d0 + 45.d0*tmp_mu**2)*Erf(1.d0/tmp_mu))))/(tmp_c**2*(6.d0*tmp_mu**4 - 12.d0*tmp_mu**6 +                      &
         dexp(1.d0/tmp_mu**2)*(-1.d0 - 18.d0*tmp_mu**4 + 12.d0*tmp_mu**6 + 10.6347231054331d0*tmp_mu**3*Erf(1.d0/tmp_mu))))) 
